      ...
      <div>
        <div className="mt-4 flex justify-between">
          {question.answers.map((answer, key) => (
            <label className="label cursor-pointer" key={key} style={{background: myAnswer !== null && (answer.isCorrect && '#057240'),borderRadius:'6px'}}>
              <input
                type="radio"
                name="answer"
                className={`radio ${
              ...
              />
              <span className="label-text ml-2" style={{color: myAnswer !== null && (answer.isCorrect && '#fff' )}}>{answer.answer}</span>
            </label>
          ))}
        </div>
      </div>
